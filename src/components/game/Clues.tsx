...
import { Lock, X, Eye, Play } from "lucide-react";
        ...
        {clues.map((clue) => (
          <button
            ...
            className={cn(
              ...
              clue.status === "active" &&
                "bg-[#1a1a1a] border-2 border-spotydle shadow-[0_0_20px_rgba(233,64,150,0.5)] animate-pulse",
              clue.status === "failed" &&
                "border-guess-wrong/50 shadow-[0_0_15px_rgba(239,68,68,0.25)]",
              clue.status === "partial" &&
                "border-guess-partial/50 shadow-[0_0_15px_rgba(234,179,8,0.25)]",
              clue.status === "completed" &&
                "border-guess-correct/50 shadow-[0_0_15px_rgba(34,197,94,0.15)] text-white",
            )}
          >
            <div
              className={cn(
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              {(clue.status === "failed" || clue.status === "partial") &&
                clue.userGuess && (
              ...
              {clue.status === "active" && clue.type === "audio" && (
                <div className="flex items-center gap-3">
                  <div className="px-1">
                    <Play
                      size={16}
                      className="text-spotydle animate-bounce"
                      fill="currentColor"
                    />
                  </div>
                </div>
              ...
              {(clue.status === "active" ||
                ...
                clue.type === "visual" && (
                  <div className="flex items-center gap-1.5 bg-black/40 hover:bg-black/60 px-2 py-1 rounded-lg border border-white/10 transition-colors">
                    <Eye size={18} className="text-spotydle" />
